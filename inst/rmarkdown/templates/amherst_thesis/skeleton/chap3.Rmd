...
Here is a reference to the Amherst logo: `r ref("amherst")`.  Note the use of the inline **R** code here.  By default "figure" is specified as the type.  For clarity, we could have also added the `label` and `type` to the parameter specifications and this would give us the same result: `r ref(label = "amherst", type = "figure")`.
