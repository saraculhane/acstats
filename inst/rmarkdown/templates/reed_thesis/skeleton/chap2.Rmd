...
Now that we've gone over some of the basics of getting used to using R Markdown in [](#rmd-basics), we will explore how to reference different spots in your document by using labels.  Anytime you enter a hashtag (or multiple hashtags) to begin a section or subsection, Markdown will automatically create a label for that section.  For example, in [](#rmd-basics), we had a section called "Additional resources."  Markdown created a label automatically (replacing spaces with hyphens).  We can create a link to this section (which will automatically correspond to the appropriate number of the section and also add the word "Section"): [](#additional-resources).
...
R Markdown uses pandoc (<http://pandoc.org/>) to build bibliographies and it requires that you don't have to do a second compile to load in references as standard \LaTeX\ requires.
