...
- Select the following three variables from the `flights` dataset:  `carrier` and `arr_delay`.  Assign this subset to a new variable called `flights2`. 

- Using `flights2`, determine the largest arrival delay for each of the carriers?  (If you'd like a reference to what the carrier abbreviations stand for, you can find them in the `airlines` data set in `pnwflights14`.)

```{r problem3}
flights2 <- flights %>% select(carrier, arr_delay)
