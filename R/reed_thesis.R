...
reed_thesis <- function() {
  ...
  # thesis.tex and use pandoc to build pdf from that
  ...
  base$post_processor <- function(metadata, utf8_input, output_file, clean, verbose) {
    ...
    wrapper_template <- find_resource("reed_thesis", "thesis.tex")
    wrapper_output <- file.path(getwd(), "thesis.tex")
    template_pandoc(wrapper_metadata, wrapper_template, wrapper_output, verbose)

    tools::texi2pdf("thesis.tex", clean = clean)
    "thesis.pdf"
  }
